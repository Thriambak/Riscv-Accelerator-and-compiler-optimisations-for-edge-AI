...
module vector_lsu (
    ...
    logic [5:0] total_store_bytes;
    assign total_store_bytes = vl_i << vsew_i;
    assign store_cycles = ((total_store_bytes + 3) >> 2) - 1;

    assign vs3_addr_o = vr_addr_i + store_cycles_cnt;
    always_ff @(posedge clk, negedge n_reset) begin
        if(~n_reset)
            store_cycles_cnt <= 3'd0;
        else if(au_start)
            store_cycles_cnt <= 3'd0;
        else if (store_cycles_inc)
            store_cycles_cnt <= store_cycles_cnt + 3'd1;
    end
