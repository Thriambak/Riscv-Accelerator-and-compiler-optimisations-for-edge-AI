module vector_registers (
    ...
    output logic [31:0] v0_mask,     // Mask register (v0) for predicated execution
    ...
    input wire n_reset,
    input wire write,
    input wire mask_enable,          // When 1, use v0 bits to gate writeback
...
// Mask register: v0 is always readable for predicated execution
assign v0_mask = vregs[0];

// Mask bits for each write group (based on element position)
// For 8-bit: each byte gets one mask bit from v0
// For 16-bit: each halfword gets one mask bit
// For 32-bit: each word gets one mask bit
logic [3:0] mask_bits;
always_comb begin
    if (!mask_enable) begin
        mask_bits = 4'b1111; // No masking, all enabled
    end else begin
        // v0 bit positions depend on which cycle we're in
        // For 8-bit SEW: bits [3:0] for first group, [7:4] for second, etc.
        mask_bits = v0_mask[3:0]; // Simplified: uses v0[3:0] for masking
    end
end
