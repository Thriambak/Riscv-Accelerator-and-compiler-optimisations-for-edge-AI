  ...
  while (getline(&line, &line_sz, fprogs) > 0) {
    ...
    // simulate program execution
    {
      ...
      while (end_cnt < extra_cycles) {
        sim_cycle++;
        // Per-cycle debug output disabled for cleaner benchmark runs
        // if (sim_cycle % 100000 == 0 || sim_cycle > 27200000) { ... }

        // read memory request
        int addr = (top->mem_addr_o % mem_sz) & ~(mem_w / 8 - 1);
        if (top->mem_req_o && top->mem_we_o) {
          if (top->mem_addr_o == 0x10000000) {
            putchar((char)(top->mem_wdata_o & 0xff));
            fflush(stdout);
          } else if (top->mem_addr_o != 0x20000004) {
            for (i = 0; i < mem_w / 8; i++)
              if ((top->mem_be_o & (1 << i)))
        ...
        if (end_cnt > 0 ||
            (top->mem_req_o == 1 &&
             (top->mem_addr_o == 0 || top->mem_addr_o == 0x20000004))) {
          if (top->mem_req_o == 1 && top->mem_addr_o == 0x20000004 &&
              end_cnt == 0) {
            printf("\n[Simulator] Exit code: %d\n", (int)top->mem_wdata_o);
          }
          end_cnt++;
        }
      }
    ...
    // write dump file and print performance metrics
    {
      ...
      // Read performance counters from memory
      // In our assembly, perf_start and perf_end are the first two words at
      // dump_start
      uint32_t perf_start_cycles =
          mem[dump_start] | (mem[dump_start + 1] << 8) |
          (mem[dump_start + 2] << 16) | (mem[dump_start + 3] << 24);
      uint32_t perf_end_cycles =
          mem[dump_start + 4] | (mem[dump_start + 5] << 8) |
          (mem[dump_start + 6] << 16) | (mem[dump_start + 7] << 24);
      uint32_t elapsed = perf_end_cycles - perf_start_cycles;

      printf("\n>>> =======================================\n");
      printf(">>> Benchmark: %s\n", prog_path);
      printf(">>> Perf Start: %u cycles\n", perf_start_cycles);
      printf(">>> Perf End:   %u cycles\n", perf_end_cycles);
      printf(">>> Elapsed:    %u cycles\n", elapsed);
      printf(">>> =======================================\n\n");
